...
class CayleyOrthogonal(nn.Module):
    ...
    baseline: dense S  (bandwidth=None)
    variant : banded sparse S (bandwidth=bw)
    """
    def __init__(self, head_dim: int, num_heads: int, eps: float = 1e-5, bandwidth=None):
        super().__init__()
        self.head_dim = head_dim
        self.num_heads = num_heads
        self.eps = eps
        self.bandwidth = bandwidth  # None => dense baseline
        ...
        if bandwidth is None:
            # dense base
            mask = torch.ones(head_dim, head_dim, dtype=torch.bool)
        else:
            mask = self._build_band_mask(head_dim, bandwidth)
    ...
    @staticmethod
    def _build_band_mask(D: int, bw: int) -> torch.Tensor:
        """
        Build a (D,D) band mask with |i-j| <= bw, diagonal excluded.
        """
    ...
    def _skew(self) -> torch.Tensor:
        """
        Apply band mask and enforce skew-symmetry.
        Returns S of shape (H, D, D).
        """
        A = self.S_raw
        mask = self.S_mask.to(A.device)

        A_masked = A * mask
        S = A_masked - A_masked.transpose(-1, -2)
        return S
...
class CayleyStringAttention(nn.Module):
    def __init__(self, hidden_size: int, num_heads: int, qkv_bias: bool = False,
                 rope_theta: float = 10.0, cayley_bandwidth=None):
        super().__init__()
...
def main():
    ...
    parser.add_argument("--mode", type=str, default="baseline", choices=["baseline", "variant"],
                        help="baseline=dense S, variant=banded sparse S")
    parser.add_argument("--bandwidth", type=int, default=4,
                        help="bandwidth for variant (ignored for baseline)")
    ...
    config = {
        ...
        "cayley_bandwidth": None if args.mode == "baseline" else args.bandwidth,
    }
    ...
    meta = {
        ...
        "mode": args.mode,
        "bandwidth": None if args.mode == "baseline" else args.bandwidth,
        "epochs": args.epochs,
