...
class CayleyOrthogonal(nn.Module):
    ...
    baseline: dense S  (topk=None)
    variant : learnable Top-k sparse S (topk=int)

    Implementation:
      - hard Top-k mask on |S_raw| per head (no straight-through)
      - exclude diagonal from top-k (diagonal cancels in skew anyway)
      - symmetrize mask for stability
      - enforce skew symmetry: S = A - A^T
      - Cayley: (I - S)(I + S)^{-1} via linear solve
    """
    def __init__(self, head_dim: int, num_heads: int, eps: float = 1e-5, topk=None):
        super().__init__()
        self.head_dim = head_dim
        self.num_heads = num_heads
        self.eps = eps
        self.topk = topk  # None => dense baseline
    ...
    def _topk_mask(self, A: torch.Tensor, k: int) -> torch.Tensor:
        """
        A: (H, D, D)
        Return hard mask in {0,1} with exactly k ones per head
        (after excluding diagonal). Then symmetrize.
        """
        H, D, _ = A.shape
        total = D * D

        if k is None:
            return torch.ones_like(A, dtype=A.dtype)

        # exclude diagonal
        absA = A.abs().clone()
        diag = torch.arange(D, device=A.device)
        absA[:, diag, diag] = -1.0  # make diagonal never selected

        eligible = total - D
        k = int(k)
        k = max(1, min(k, eligible))  

        flat = absA.reshape(H, total)            
        _, idx = torch.topk(flat, k, dim=-1)     

        mask_flat = torch.zeros(H, total, dtype=torch.bool, device=A.device)
        mask_flat.scatter_(1, idx, True)
        mask = mask_flat.view(H, D, D)           

        # Symmetrize and clear diagonal
        mask = mask | mask.transpose(-1, -2)
        mask[:, diag, diag] = False

        return mask.to(dtype=A.dtype)
    ...
    def _skew(self) -> torch.Tensor:
        ...
        if self.topk is None:
            ...
            mask = self._topk_mask(A, self.topk)
            A_masked = A * mask

        #skew-symmetry
        return A_masked - A_masked.transpose(-1, -2)
...
class CayleyStringAttention(nn.Module):
    def __init__(self, hidden_size: int, num_heads: int, qkv_bias: bool = False,
                 rope_theta: float = 10.0, cayley_topk=None):
        super().__init__()
        ...
        self.cayley = CayleyOrthogonal(
            ...
            topk=cayley_topk,  #none = dense, int = topk
        )
...
class Block(nn.Module):
    def __init__(self, config):
        ...
        self.attention = CayleyStringAttention(
            ...
            cayley_topk=config.get("cayley_topk", None),
        )
...
def main():
    ...
    parser.add_argument("--mode", type=str, default="baseline", choices=["baseline", "variant"],
                        help="baseline=dense S, variant=Top-k sparse S")

    parser.add_argument("--topk", type=int, default=48,
                        help="Top-k entries kept per head in S_raw for variant (ignored for baseline). ")
    ...
    config = {
        ...
        "cayley_topk": None if args.mode == "baseline" else args.topk,
    }
    ...
    if args.mode == "variant":
        tag += f"-topk{args.topk}"

    save_best = f"/content/drive/MyDrive/final/{tag}-best.pt"

    meta = {
        "dataset": args.dataset,
        "mode": args.mode,
        "topk": None if args.mode == "baseline" else args.topk,
        "epochs": args.epochs,
